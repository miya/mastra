...
<TSDocPropertiesTable 
  definition={generateDefinition({
    code: `import type { AgentExecutionOptions } from '@mastra/core/agent'
    export default AgentExecutionOptions
    `
  })}
/>
...
{/* ### TSDoc Test - AgentExecutionOptions
...
<PropertiesTable
  content={[
    {
      name: "MastraModelOutput<Output>",
      type: "Streaming interface",
      description: "Returned when format is 'mastra' or undefined. Mastra's native streaming interface with comprehensive access to all stream data and methods."
      
    },
    {
      name: "AISDKV5OutputStream<Output>",
      type: "Streaming interface", 
      description: "Returned when format is 'aisdk'. Compatible with AI SDK v5 interfaces like useChat, providing methods like toUIMessageStreamResponse() and toTextStreamResponse()."
    }
  ]}
/>

The return type depends on the `format` parameter:
- **`'mastra'` (default)**: Returns [`MastraModelOutput<Output>`](./streamVNextTypes/MastraModelOutput)
- **`'aisdk'`**: Returns [`AISDKV5OutputStream<Output>`](./streamVNextTypes/AISDKV5OutputStream)

## Stream interfaces

The stream object returned provides different interfaces based on the format:

### MastraModelOutput (format: 'mastra')

The default format returns a [`MastraModelOutput`](./streamVNextTypes/MastraModelOutput) object with comprehensive streaming capabilities:

```typescript
const stream = await agent.streamVNext("Hello");
// stream is MastraModelOutput<Output>
...
### AISDKV5OutputStream (format: 'aisdk')

The AI SDK format returns an [`AISDKV5OutputStream`](./streamVNextTypes/AISDKV5OutputStream) object optimized for frontend integration:

```typescript
const stream = await agent.streamVNext("Hello", { format: 'aisdk' });
// stream is AISDKV5OutputStream<Output>
...
Both stream types share these promise-based accessors:
- `stream.text` - Complete text response
- `stream.usage` - Token usage statistics  
- `stream.reasoning` - Reasoning text (if supported)
- `stream.toolCalls` - Array of tool calls made
- `stream.toolResults` - Array of tool results
- `stream.object` - Structured output object

## Stream types reference

For detailed information about the stream interfaces and chunk types:

- [MastraModelOutput](./streamVNextTypes/MastraModelOutput) - Complete reference for the Mastra format stream
- [AISDKV5OutputStream](./streamVNextTypes/AISDKV5OutputStream) - Complete reference for the AI SDK format stream  
- [ChunkType](./streamVNextTypes/ChunkType) - All possible chunk types emitted during streaming
