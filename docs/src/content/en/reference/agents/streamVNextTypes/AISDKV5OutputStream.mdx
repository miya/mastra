...
## How to Get an AISDKV5OutputStream

The `AISDKV5OutputStream` is returned when you call `streamVNext()` with `format: 'aisdk'`:

```typescript
const stream = await agent.streamVNext("Generate a response", {
  format: 'aisdk'
});
// stream is now an AISDKV5OutputStream instance
